...
class OneClickDemoImport {
  ...
  private $plugin_page_setup = array();
		...
		if ( $this->plugin_page === $hook || ( 'admin.php' === $hook && $this->plugin_page_setup['menu_slug'] === esc_attr( $_GET['import'] ) ) ) {
			...
			wp_localize_script( 'ocdi-main-js', 'ocdi',
				array(
					...
					'texts'            => array(
						'missing_preview_image' => esc_html__( 'No preview image defined for this import.', 'ahura' ),
						'dialog_title'          => esc_html__( 'Are you sure?', 'ahura' ),
						'dialog_no'             => esc_html__( 'Cancel', 'ahura' ),
						'dialog_yes'            => esc_html__( 'Yes, import!', 'ahura' ),
						'selected_import_title' => esc_html__( 'Selected demo import:', 'ahura' ),
					),
	...
	public function import_demo_data_ajax_callback() {
		...
		if ( ! $use_existing_importer_data ) {
			...
			if ( ! empty( $_FILES ) ) { // Using manual file uploads?
				...
				$this->import_files[ $this->selected_index ]['import_file_name'] = esc_html__( 'Manually uploaded files', 'ahura' );
			}
			elseif ( ! empty( $this->import_files[ $this->selected_index ] ) ) { // Use predefined import files from wp filter: pt-ocdi/import_files.
				...
				if ( is_wp_error( $this->selected_import_files ) ) {
					...
					Helpers::log_error_and_send_ajax_response(
						...
						$this->log_file_path,
						esc_html__( 'Downloaded files', 'ahura' )
					);
				}
				...
				$log_added = Helpers::append_to_file(
					sprintf(
						__( 'The import files for: %s were successfully downloaded!', 'ahura' ),
				...
				wp_send_json( esc_html__( 'No import files specified!', 'ahura' ) );
			}
		}
	...
	private function final_response() {
		...
		if ( empty( $this->frontend_error_messages ) ) {
			...
			if ( ! apply_filters( 'pt-ocdi/disable_pt_branding', false ) ) {
				$twitter_status = esc_html__( 'Just used One Click Demo Import plugin and it was awesome! Thanks @mihanwp! #wordpress https://mihanwp.com/', 'ahura' );

				$response['message'] .= sprintf(
					__( '%1$s%6$sWasn\'t this a great One Click Demo Import experience?%7$s Created and maintained by %3$sMihanWP%4$s. %2$s%5$sClick to Tweet!%4$s%8$s', 'ahura' ),
					'<div class="notice  notice-info"><p>',
					'<br>',
					'<strong><a href="https://mihanwp.com/" target="_blank">',
					'</a></strong>',
			...
			$response['message'] .= sprintf(
				__( 'Done! Enjoy from your Website.', 'ahura' ),
			...
			$response['message'] .= sprintf(
				__( '%1$sThe demo import has finished.%2$s', 'ahura' ),
				'<div class="notice  notice-success"><p>',
				'</p></div>'
			);
		}
	...
	public function load_textdomain() {
		load_plugin_textdomain( 'ahura', false, plugin_basename( dirname( dirname( __FILE__ ) ) ) . '/languages' );
	}
